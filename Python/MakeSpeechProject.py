...
retCode = os.system("python SpeakPythonMakeDB.py " + appName + " " + spsName);
...
retCode = os.system("python SpeakPython/SpeakPythonMakeJSGF.py " + appName + " " + spsName);
...
retCode = os.system("cp SpeakPython/pocketsphinx.dic " + appName + ".dic");
if retCode != 0:
	print "Failed to copy pocketsphinx default dictionary from SpeakPython/pocketsphinx.dic.";
	sys.exit(4);	
...
if os.path.isfile("grammar_words_temp.txt"):
	if newWordsExist:
		...
		retCode = os.system("g2p.py --model SpeakPython/model-5 --apply " + wordsFile + " >> " + appName + ".dic && rm " + wordsFile);
	else:
